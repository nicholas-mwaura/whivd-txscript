// Copyright (c) 2013-2017 The nicholas-mwaura developers
...
// newAddressPubKey returns a new whiveutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) whiveutil.Address {
	addr, err := whiveutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new whiveutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) whiveutil.Address {
	addr, err := whiveutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new whiveutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) whiveutil.Address {
	addr, err := whiveutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the whiveutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the whiveutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := whiveutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := whiveutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := whiveutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := whiveutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := whiveutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*whiveutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*whiveutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*whiveutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*whiveutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*whiveutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
