...
// specified address.
func PayToAddrScript(addr whiveutil.Address) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"

	switch addr := addr.(type) {
	case *whiveutil.AddressPubKeyHash:
		if addr == nil {
	...
	case *whiveutil.AddressScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *whiveutil.AddressWitnessPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
		return payToWitnessPubKeyHashScript(addr.ScriptAddress())
	case *whiveutil.AddressWitnessScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*whiveutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []whiveutil.Address, int, error) {
	var addrs []whiveutil.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := whiveutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case WitnessV0PubKeyHashTy:
		...
		// Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := whiveutil.NewAddressWitnessPubKeyHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := whiveutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := whiveutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case WitnessV0ScriptHashTy:
		...
		// Skip the script hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := whiveutil.NewAddressWitnessScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]whiveutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := whiveutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
