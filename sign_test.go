// Copyright (c) 2013-2016 The nicholas-mwaura developers
...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr whiveutil.Address) (*btcec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr whiveutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address, err := whiveutil.NewAddressPubKeyHash(
				whiveutil.Hash160(pk), &chaincfg.TestNet4Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*btcec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := whiveutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := whiveutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk1 := (*btcec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := whiveutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet4Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			pk2 := (*btcec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := whiveutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet4Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*whiveutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
